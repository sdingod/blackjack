...
def scenario_1(balance, bet, value, s, d1, tot, RC, TC, i, decks, cards): #Scenario 1
  while True:
    if (value == "H"): #Hit
      pass
    elif (value == "S"): #Stand
    ...
    while True: #Additional cards if hit
      ...
      if (c != 11): #New card is not an ace with value 11
        s += c
        tot += c
        value = read_value("S2.txt", row_x = s, column_y = d1, TC = TC)
        if (value == "H"): #Hit
          pass
        elif (value == "S"): #Stand
          break
      ...
      if (c == 11): #New card is an ace with possible value 11
        value = read_value("S3.txt", row_x = s, column_y = d1, TC = TC)
        if (value == "H"): #Hit
          pass
        elif (value == "S"): #Stand
          tot += c
...
def scenario_2(balance, bet, s, d1, tot, RC, TC, i, decks, cards): #Scenario 2
  while True:
    value = read_value("S3.txt", row_x = s, column_y = d1, TC = TC)
...
def scenario_3(balance, bet, c1, d1, RC, TC, i, decks, cards, totar, betar, zar): #Scenario 3
  balance -= bet
  for x in range (2):
    c, RC, i, TC = card(RC, i, decks, cards)
    bet = bet1
    ...
    if (c1 != 11  and c != 11 and c1 != c): #Scenario 1
      s = c1 + c
      tot = s
      value = read_value("S2.txt", row_x = s, column_y = d1, TC = TC)
      balance, bet, d1, tot, RC, TC, i = scenario_1(balance, bet, value, s, d1, tot, RC, TC, i, decks, cards)
      totar.append(tot)
      betar.append(bet)
      zar.append(z)
    ...
    if ((c1 == 11 or c == 11) and c1 != c and (c1 + c) != 21): #Scenario 2
      if (c1 == 11): #Card 1 is an ace
        ...
        s = c1
        tot = c1
      balance, bet, d1, tot, RC, TC, i = scenario_2(balance, bet, s, d1, tot, RC, TC, i, decks, cards)
      totar.append(tot)
      betar.append(bet)
      zar.append(z)

    if (c1 == c): #Scenario 3
      balance, betar, d1, totar, RC, TC, i, zar = scenario_3(balance, bet, c1, d1, RC, TC, i, decks, cards, totar, betar, zar)

  return balance, betar, d1, totar, RC, TC, i, zar
  ...
  while True: #Cycle every time a play is made
    ...
    if ((d1 + d2) == 21 and (c1 + c2) != 21):
      ...
      if (c1 != c2 and c1 != 11 and c2 != 11):
        ...
        #if ((s == 16 and (d1 == 9 or d1 == 10 or d1 == 11)) or (s == 15 and d1 == 10)):
        #  value = "U"
        #  balance += bet/2
        #  surr += 1
        #  pass
      ...
      #Scenario 3
      if (c1 == c2):
        value = read_value("S1.txt", row_x = c1, column_y = d1, TC = TC)
        if (value == "Y"): #Split
          balance, betar, d1, totar, RC, TC, i, zar = scenario_3(balance, bet, c1, d1, RC, TC, i, decks, cards, totar, betar, zar)
    ...
    #Dealer cards, soft 17 (only if not bj or surr)
    check = 0
    for k in range (len(zar)):
      if totar[k] != 0:
        check = 1

    if (check == 1 and surr == surr1):
      if (d1 == 11 or d2 == 11):
        if (d1 == 11 and d2 == 11):
          dtot = 1
    ...
    for k in range (len(totar)):
      if (totar[k] != 0 and dtot < 22 and surr == surr1 and totar[k] > dtot):
        balance += 2*betar[k]
        w += 1
      elif (totar[k] != 0 and dtot < 22 and surr == surr1 and totar[k] < dtot):
        l += 1
      elif (totar[k] != 0 and dtot < 22 and surr == surr1 and totar[k] == dtot):
        balance += betar[k]
        dr += 1
    
    #Used for not dealing cards if surrenderd
    surr1 = surr
...
#print ("Win prob: ", w/(w+l+bj+surr+dr)*100)
#print ("Loss prob: ", l/(w+l+bj+surr+dr)*100)
print ("Blackjack prob: ", bj/(w+l+bj+surr+dr)*100)
#print ("Surrender prob: ", surr/(w+l+bj+surr+dr)*100)
#print ("Draw prob: ", dr/(w+l+bj+surr+dr)*100)

print ("Win: ", (w+bj)/(w+l+bj+surr+dr)*100)
print ("Loss: ", (l + surr)/(w+l+bj+surr+dr)*100)
print ("Draw: ", dr/(w+l+bj+surr+dr)*100)

print (w - l + bj*1.5 - surr*0.5)
